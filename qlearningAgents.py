...
class QLearningAgent(ReinforcementAgent):
    ...
    def __init__(self, **args):
        ...
        "*** YOUR CODE HERE ***"
        self.q_value_table = defaultdict(float)
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in possible_actions: 
            q_value_for_action= self.getQValue(state, action)
            if q_value_for_action > highest_q_value: 
                highest_q_value = q_value_for_action
                best_actions = [action] 
            elif q_value_for_action == highest_q_value: 
    ...
    def update(self, state, action, nextState, reward):
        ...
        "*** YOUR CODE HERE ***"
        #Current q-value
        existing_q_value = self.getQValue (state,action)
        #Maximum q-value for next state
        future_q_value= self.computeValueFromQValues(nextState)
        #Update Rule
        updated_q_value = ((1-self.alpha)* existing_q_value + self.alpha * (reward + self.discount * future_q_value))
        self.q_value_table [(state,action)]= updated_q_value
